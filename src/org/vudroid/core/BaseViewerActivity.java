...
import com.fax.weihanstudy.StartScreen;
...
public abstract class BaseViewerActivity extends StatActivity implements DecodingProgressListener, CurrentPageListener
{
    private static final int MENU_EXIT = 0;
    private static final int MENU_GOTO = 1;
    private static final int MENU_FULL_SCREEN = 2;
    private static final int MENU_FASTSEARCH = 3;
...
//        frameLayout.addView(createZoomControls(zoomModel));
    ...
    {
        menu.add(0, MENU_GOTO, 0, "页面跳转");
        menu.add(0, MENU_FASTSEARCH, 0, "打开词典");
        final MenuItem menuItem = menu.add(0, MENU_FULL_SCREEN, 0, "全屏").setCheckable(true).setChecked(viewerPreferences.isFullScreen());
        setFullScreenMenuItemText(menuItem);
        menu.add(0, MENU_EXIT, 0, "退出阅读");
        return true;
    }

    private void setFullScreenMenuItemText(MenuItem menuItem)
    {
        menuItem.setTitle("全屏 " + (menuItem.isChecked() ? "开" : "关"));
    }
